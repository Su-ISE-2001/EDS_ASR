...
if (!$env:ASR_WHISPER_MODEL) { $env:ASR_WHISPER_MODEL = "small" }
if (!$env:ASR_WHISPER_DEVICE) { $env:ASR_WHISPER_DEVICE = "cuda" }
if (!$env:ASR_WHISPER_COMPUTE_TYPE) { $env:ASR_WHISPER_COMPUTE_TYPE = "float16" }
